...
import { rid, orderPreferredOntologyTerms, convertRecordToQueryFilters } from '../graphkb';
...
const fetchAndLoadBySymbol = async ({
    ...
    try {
        const filters = {
            AND: [
                { source: { filters: { name: sourceDefns.hgnc.name }, target: 'Source' } },
            ],
        };
    ...
    if (CACHE.SOURCE) {
        ...
        hgnc = await conn.addSource(sourceDefns.hgnc);
        CACHE.SOURCE = hgnc;
    }
    let ensembl;
    ...
    try {
        ensembl = await conn.getUniqueRecordBy({
            filters: { name: sourceDefns.ensembl.name },
            target: 'Source',
        });
    } catch (err) { }
...
const uploadFile = async (opt) => {
    ...
    const hgnc = await conn.addSource(sourceDefns.hgnc);
    let ensembl;
...
export {
    fetchAndLoadBySymbol, uploadFile, uploadRecord,
};
