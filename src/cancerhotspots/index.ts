...
import kbParser from '@bcgsc-pori/graphkb-parser';
...
import _entrezGene from '../entrez/gene';
...
const processVariants = async ({ conn, record, source }) => {
    ...
    try {
        ...
        const variant = kbParser.variant.parse(notation).toJSON();
    ...
    try {
        ...
        const variant = kbParser.variant.parse(
    ...
    try {
            ...
            reference1 = rid(await conn.getUniqueRecordBy({
                filters: {
                    AND: [
                        ...
                        { biotype: 'transcript' },
                        { source: { filters: { name: sourceDefns.ensembl.name }, target: 'Source' } },
                    ],
                },
                sort: orderPreferredOntologyTerms,
                target: 'Feature',
        ...
        const variant = kbParser.variant.parse(cds, false).toJSON();
    ...
    if (diseasesCache[diseaseId]) {
        ...
        disease = rid(await conn.getUniqueRecordBy({
            filters: {
                AND: [
                    { sourceId: diseaseId },
                    { source: { filters: { name: sourceDefns.oncotree.name }, target: 'Source' } },
                ],
            },
            sort: orderPreferredOntologyTerms,
            target: 'Disease',
...
const uploadFile = async ({ filename, conn, errorLogPrefix }) => {
    ...
    const source = rid(await conn.addSource(sourceDefns.cancerhotspots));
