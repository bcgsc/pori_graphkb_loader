...
import _ensembl from '../ensembl';
import _hgnc from '../hgnc';
import { logger } from '../logging';

import sourceDefns from '../sources';


const { cosmic: SOURCE_DEFN } = sourceDefns;
const HEADER = {
    cds: 'HGVSC',
...
const processVariants = async ({ conn, record, source }) => {
    ...
    try {
        ...
        } = kbParser.variant.parse(record.protein, false);
    ...
    if (record.cds && record.cds.trim()) {
        try {
            const {
                noFeatures, multiFeature, prefix, ...variant
            } = kbParser.variant.parse(record.cds, false);
    ...
    if (record.genomic) {
        try {
            const {
                noFeatures, multiFeature, prefix, ...variant
            } = kbParser.variant.parse(record.genomic, false);
            // get the chromosome
...
export {
    loadClassifications, processDisease,
};
