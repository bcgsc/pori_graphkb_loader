 ...
 */
const getDrugOrAdd = async (conn: ApiConnection, source, name, rawRecord = {}) => {
    if (!name) {
        return null;
    }
    const tags: string[] = [];
    ...
    try {
        record = await conn.getUniqueRecordBy({
            filters: {
                AND: [
                    { source: { filters: { name: sourceDefns.drugbank.name }, target: 'Source' } },
                    { name },
                ],
            },
    ...
    try {
        record = await conn.getUniqueRecordBy({
            filters: { AND: [{ source: { filters: { name: sourceDefns.chembl.name }, target: 'Source' } }, { name }] },
            sort: orderPreferredOntologyTerms,
            target: 'Therapy',
        });
        return record;
 ...
 */
const addDrugClass = async (conn: ApiConnection, source, name, rawRecord) => {
    if (!name) {
        return null;
    }
 ...
 */
const uploadFile = async ({ filename, conn }: { filename: string, conn: ApiConnection }) => {

    const content = await loadDelimToJson(filename);

    const source = rid(await conn.addSource(sourceDefns.gscTherapeuticOntology));
