...
const convertOwlGraphToJson = (graph: {statements: OwlGraphNode[]}, idParser = x => x): {[key: string]: any} => {
...
const loadDelimToJson = async (filename: string, opt: {delim?: string; header?: boolean} = {}): Promise<any[]> => {
 ...
 */
const convertRowFields = (header: {[key: string]: string}, row: {[key: string]: any}) => {
    const result = {};
