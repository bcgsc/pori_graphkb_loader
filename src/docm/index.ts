...
import { orderPreferredOntologyTerms, rid } from '../graphkb';
import _pubmed from '../entrez/pubmed';
import { logger } from '../logging';
import _gene from '../entrez/gene';
import sourceDefns from '../sources';

const { docm: SOURCE_DEFN } = sourceDefns;
const ajv = new Ajv();
...
const processVariants = async ({ conn, source, record: docmRecord }) => {
    ...
    try {
        ...
        let variant = kbParser.variant.parse(parseDocmVariant(aminoAcid), false).toJSON();
    ...
    try {
        ...
        let variant = kbParser.variant.parse(buildGenomicVariant(docmRecord), false).toJSON();
        const type = await conn.getVocabularyTerm(variant.type);
...
export { recordSpec, variantSummarySpec };
