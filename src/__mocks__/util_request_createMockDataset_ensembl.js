...
const request = async ({
    ...
    const req = {
        url: url.href.replace(/^http.*8080/, 'http://bcgsc.ca:8080'),
    };
    ...
    const index = mockDataset.length - 1;

    if (resp.ok) {
        if (json) {
            const result = await resp.json();
            mockDataset[index] = { ...mockDataset[index], response: result };
            fs.writeFileSync(filepath, JSON.stringify(mockDataset));
            return result;
        }
        const result = await resp.text();
        mockDataset[index] = { ...mockDataset[index], response: JSON.parse(result) };
        fs.writeFileSync(filepath, JSON.stringify(mockDataset));
        return result;
    }
    const errorMessage = await resp.text();
    mockDataset[index] = { ...mockDataset[index], response: JSON.parse(errorMessage) };
    fs.writeFileSync(filepath, JSON.stringify(mockDataset));
