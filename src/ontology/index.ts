...
import { rid, convertRecordToQueryFilters } from '../graphkb';
...
const validateSpec = ajv.compile({
    properties: {
        class: {
            enum: kbSchema.schema.schema.Ontology.descendantTree(true).map(model => model.name),
            type: 'string',
        },
                    ...
                    links: {
                        items: {
                            properties: {
                                ...
                                class: { enum: kbSchema.schema.getEdgeModels().map(e => e.name), type: 'string' },
...
export { uploadFile, uploadFromJSON };
