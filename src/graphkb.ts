...
import { GraphKbRecord } from './types';
...
const rid = (record: string | object): string => {
    return (record['@rid'] || record).toString();
};


const nullableRid = (record: string | object): string | null => {
    if (!record) {
        return null;
    }
    return (record['@rid'] || record).toString();
 ...
 */
const shouldUpdate = (modelIn, originalContentIn, newContentIn, upsertCheckExclude: string[] = []) => {
...
const orderPreferredOntologyTerms = (term1, term2) => {
    ...
    if (term1.sourceId === term2.sourceId && nullableRid(term1.source) === nullableRid(term2.source)) {
...
class ApiConnection {
    ...
    async request(opt : {
        ...
        if (this.exp === null || this.exp <= epochSeconds()) {
            await this.login();
        }
        const {
     ...
     */
    async getTherapy(term: string, source: string | null = null) {
        let error,
            filters: any = {
                OR: [
                    { sourceId: term },
     ...
     */
    async getVocabularyTerm(term: string, sourceName: string = sourceDefns.graphkb.name) {
        if (!term) {
    ...
    async updateRecord(target: string, recordId: string, newContent: object) {
     ...
     */
    async deleteRecord(target: string, recordId: string) {
        const model = schema.get(target);
        const { result } = jc.retrocycle(await this.request({
            method: 'DELETE',
            uri: `${model.routeName}/${recordId.replace(/^#/, '')}`,
    ...
    async addRecord(opt: {
        ...
        sortFunc?: () => [1, 0, -1];
        upsert?: boolean;
        upsertCheckExclude?: string[]
    }): Promise<GraphKbRecord | null> {
        const {
            content,
            target,
            existsOk = false,
    ...
    async addSource(content: any, opt = {}): Promise<GraphKbRecord> {
     ...
     */
    async addVariant(opt: {content: any; target: string}) {
        const {
            content,
            target,
        } = opt;
     ...
     */
    async addTherapyCombination(source, therapyName: string, opt: {matchSource?: boolean} = {}) {
