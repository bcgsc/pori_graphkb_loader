...
import fs from 'fs';
import { convertOwlGraphToJson } from '../util';
import { orderPreferredOntologyTerms, rid, convertRecordToQueryFilters } from '../graphkb';
...
const uploadFile = async ({ filename, conn }) => {
    ...
    const subclassEdges = [];
    const source = await conn.addSource(sourceDefns.uberon);
    let ncitSource = null;

    try {
        ncitSource = await conn.getUniqueRecordBy({
            filters: { name: sourceDefns.ncit.name },
            target: 'Source',
        });
    } catch (err) {
        ...
        for (const { src, tgt } of ncitLinks) {
            ...
            try {
                const ncitRecord = await conn.getUniqueRecordBy({
                    filters: {
                        AND: [
                            { source: { filters: { name: sourceDefns.ncit.name }, target: 'Source' } },
                            { sourceId: tgt },
                        ],
                    },
