...
import { rid, convertRecordToQueryFilters, orderPreferredOntologyTerms } from '../graphkb';
 ...
 */
const cleanRawRow = (rawRow: any): NormalizedNcitRow => {
    const {
        id,
...
const uploadFile = async ({ filename, conn, ignoreCache = false }) => {
    ...
    const deprecatedRows: NormalizedNcitRow[] = [];
    ...
    const source = rid(await conn.addSource(sourceDefns.ncit));
