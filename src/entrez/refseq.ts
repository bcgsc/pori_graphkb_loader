...
const fetchAndLoadByIds = async (api, idListIn) => {
    ...
    if (versionedIds.length > 0) {
        const fullRecords = await util.fetchByIdList(
            versionedIds,
            {
    ...
    const result = await Promise.all(records.map(rec => limit(() => util.uploadRecord(api, rec, {
        cache: CACHE,
...
const fetchAndLoadBySearchTerm = (api, term, opt = {}) => util.fetchAndLoadBySearchTerm(api, term, {
    ...opt,
    cache: CACHE,
    dbName: DB_NAME,
    parser: parseRecord,
