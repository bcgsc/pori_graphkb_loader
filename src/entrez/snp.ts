...
const { dbSnp: SOURCE_DEFN } = sourceDefns;
const ajv = new Ajv();
...
const loadFromDocsumHgvs = async (api, hgvsVariants) => {
    ...
    try {
        if (hgvsVariants.cds) {
            const parsed = kbParser.variant.parse(hgvsVariants.cds.split('|')[0], true).toJSON();
    ...
    try {
        if (hgvsVariants.protein) {
            ...
            const parsed = kbParser.variant.parse(hgvsVariants.protein.split('|')[0], true).toJSON();
...
const loadSnpRecord = async (api, { hgvs, genes, ...content }) => {
    const uploaded = await util.uploadRecord(api, content, {
        cache: CACHE,
...
const fetchAndLoadByIds = async (api, idListIn) => {
    const records = await util.fetchByIdList(
        idListIn,
        {
            cache: CACHE, db: DB_NAME, parser: parseRecord, url: util.BASE_FETCH_URL,
        },
    );
